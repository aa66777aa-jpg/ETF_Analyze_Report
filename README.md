...
RSI 與 Williams %R 是相對指標，不受槓桿倍數影響。`leverage_list` 中的代號同時也要出現在 `stock_list`。
...
反向 ETF 的評分邏輯全部翻轉：市場過熱（RSI 高、接近高點、高於 MA60、W%R 超買）才是加碼時機；市場大跌（RSI 低、遠離高點）反而是暫緩訊號。`inverse_list` 中的代號同時也要出現在 `stock_list`。
