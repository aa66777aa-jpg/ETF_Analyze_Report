...
- 低於 MA60 超過 5% 且 **均線向上**（近 10 日斜率為正）→ 回檔加碼機會 → **+1 分**
...
單純比較 CMF 絕對值容易誤判：賣壓仍在加速流出時就判定「加碼」等於接刀，買盤仍強勁流入時就判定「暫緩」則可能提早喊停一段還在走的行情。因此除了跨越閾值，還要求**力竭確認**——CMF 相對 `CMF_REVERSAL_WINDOW`（預設 5）日前已經開始反轉，且同一期間價格尚未搶先反應，才真正計分，否則僅標記為觀察中（0 分）：

- CMF ≤ -0.15（資金流出區間）**且** 5 日來 CMF 已回升（流出趨緩）**且** 收盤價 5 日來未見反彈（仍偏弱）→ 賣壓力竭 → **+1 分**
- CMF ≤ -0.15 但尚未同時滿足上述兩個力竭條件 → 資金仍在流出、尚未止穩 → **0 分（觀察中）**
- CMF ≥ 0.15（資金流入區間）**且** 5 日來 CMF 已回落（流入趨緩）**且** 收盤價 5 日來未見拉回（仍在高檔）→ 買盤力竭 → **-1 分**
...
>
> 反轉回看天數（`CMF_REVERSAL_WINDOW`）刻意選得比 CMF 本身的計算週期（`CMF_PERIOD`，20 日）短：若兩者相同，「相對 N 日前回升」會因為滾動窗口本身的長度，幾乎不可能與「仍在超賣/超買區」同時成立——實測以 5.6 年歷史資料回測 0050.TW、00735.TW，用 20 日窗口時力竭確認整段期間完全零觸發；換成 5 日窗口後才恢復到合理的觸發頻率（同期各有數次至十餘次），仍維持稀少但不再形同虛設。
